...
import { inputTopic, type MessageKey, type MessageValue, outputTopic } from './key-filter.fixtures';

@Controller()
export class KeyFilterController {
  constructor(@Inject(KAFKA_SERVICE) private readonly messageService: MessageService) {}

  @ConsumeTopic(inputTopic)
  async test1(@Payload() message: Message<MessageKey, MessageValue>): Promise<void> {
